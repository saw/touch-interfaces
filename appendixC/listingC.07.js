(function(){
	...
	var homeTemplate = Handlebars.compile(
	    document.getElementById('t-home').innerHTML
	    );
	var quizTemplate = Handlebars.compile(
	    document.getElementById('t-quiz').innerHTML
	    );
	...
	quizModel.setUpQuestion = function() {
		...
		list.sort(function(){
			//array.sort takes an optional comperator
			//If the return value is positive nothing happens,
			//if they are negative they are switched.
			
			//The result of this is that the array is randomized. Because
			//Math.random is a value between 0 and one, but rounded it is 
			//either 0 or one, subtracting 0.5 results in either -0.5 or 0.5
